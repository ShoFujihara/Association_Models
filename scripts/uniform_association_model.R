...
fitted_1 %<>% dplyr::select(O, D, Freq, Fitted_1 = .fitted)
fitted_2 <- augment(fit_2, type.predict = "response", newdata = df)
fitted_2 %<>% dplyr::select(O, D, Freq, Fitted_2 = .fitted)
fitted_3 <- augment(fit_3, type.predict = "response", newdata = df)
fitted_3 %<>% dplyr::select(O, D, Freq, Fitted_3 = .fitted)
fitted_4 <- augment(fit_4, type.predict = "response", newdata = df)
fitted_4 %<>% dplyr::select(O, D, Freq, Fitted_4 = .fitted)
fitted_5 <- augment(fit_5, type.predict = "response", newdata = df)
fitted_5 %<>% dplyr::select(O, D, Freq, Fitted_5 = .fitted)
...
obs <- fit_df %>% xtabs(Freq ~ O + D, data = .)
t_fitted_1 <- fit_df %>% xtabs(Fitted_1 ~ O + D, data = .)
t_fitted_2 <- fit_df %>% xtabs(Fitted_2 ~ O + D, data = .)
t_fitted_3 <- fit_df %>% xtabs(Fitted_3 ~ O + D, data = .)
t_fitted_4 <- fit_df %>% xtabs(Fitted_4 ~ O + D, data = .)
t_fitted_5 <- fit_df %>% xtabs(Fitted_5 ~ O + D, data = .)
